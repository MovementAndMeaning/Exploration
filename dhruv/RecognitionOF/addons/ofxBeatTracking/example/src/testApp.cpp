...
void testApp::setup(){
    ...
    ofSoundStreamSetup(0, 1, this, 44100, bd.getBufferSize(), 4);
...
void testApp::draw(){
	...
	int testApp_fft_size = bd.getFftSize();
    ...
    for (int i = 1; i < (int)testApp_fft_size / 2; i++){
        ...
        ofLine(10 + (i * 3), 150, 10 + (i * 3), 150 - bd.getMagnitude(i) * 10.0f);
        //printf("%f \n", magnitude_average[i]);
    }
    for (int i = (int)testApp_fft_size / 2, b = 1; i < testApp_fft_size; i++){
        ...
        ofLine(10 + (b * 3), 300, 10 + (b * 3), 300 - bd.getMagnitude(i) * 10.0f);
        b++;
        //printf("%f \n", magnitude_average[i]);
    }
    ...
    for (int i = 1; i < (int)testApp_fft_size/2; i++){
        ...
        ofLine(10 + (i * 3), 500, 10 + (i * 3), 500 - bd.getMagnitudeAverage(i) * 10.0f);
    }
    for (int i = (int)testApp_fft_size / 2, b = 1; i < testApp_fft_size; i++){
        if(i % 16 == 0) {
            ofSetColor(200,0,0);
        ...
        ofLine(10 + (b * 3), 650, 10 + (b * 3), 650 - bd.getMagnitudeAverage(i) * 10.0f);
        b++;
    }
